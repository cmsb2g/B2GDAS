...
parser.add_option('--systematicsJEC', type='int', action='store',
                  default=0,
                  dest='systematicsJEC',
                  help='0 nominal, 1 up, -1 down')

parser.add_option('--systematicsJER', type='int', action='store',
                  default=0,
                  dest='systematicsJER',
                  help='0 nominal, 1 up, -1 down')

parser.add_option('--systematicsBTAG', type='int', action='store',
                  default=0,
                  dest='systematicsBTAG',
                  help='0 nominal, 1 up, -1 down')
...
jecParStrAK4 = ROOT.std.string('JECs/PHYS14_25_V1_Uncertainty_AK4PFchs.txt')
jecUncAK4 = ROOT.JetCorrectionUncertainty( jecParStrAK4 )
jecParStrAK8 = ROOT.std.string('JECs/PHYS14_25_V1_Uncertainty_AK8PFchs.txt')
jecUncAK8 = ROOT.JetCorrectionUncertainty( jecParStrAK8 )
#jecUncAK8 = ROOT.JetCorrectionUncertainty( 'JECs/PHYS14_25_V1_Uncertainty_AK8PFchs.txt' )
...
for ifile in files :
    ...
    for event in events:
        ...
        if nttags == 0 :
            ...
            # Add the B-tagging Scale factor. If uncertainty is "up" or "down", also
	    ...
	    SFb = getBtagSF(bJetCandP4.Perp())
	    SFb_error = getBtagSFerror(bJetCandP4.Perp())
	
	    if options.systematicsBTAG == 0:
		SF = SFb
	
	    if options.systematicsBTAG == 1:
		SF = SFb + SFb_error
	
	    if options.systematicsBTAG == -1:
		SF = SFb - SFb_error	

	    bdisc = ak4JetsGood[inearestJet].bDiscriminator( options.bdisc )
